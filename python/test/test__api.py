...
def before_call(context, case):
    ...
    # debug output
    # ------------
    #full_url = f"{case.base_url}{case.formatted_path}"
    print(f"{case.formatted_path}")

    print(f"   Query Parameters: {case.query}")

    body = case.body if type(case.body) is not schemathesis.types.NotSet else None
    print(f"   Body: {body}")
    # ----------------------------------------------------------------------
